...
function setupTransaksiPage() {
	...
	function tampilkanDaftarTransaksi(){
        ...
        <div class="flex columns relative">
        	<strong>${tx.kategori}</strong> - ${tx.tanggal}
					<span>${tx.jenis === "pemasukan" ? "+" : "-"} Rp${tx.nominal.toLocaleString()}</span>
					<em>${tx.catatan || ""}</em>
      ...
      `;
		...
		saldo.textContent = `Rp ${(pemasukan - pengeluaran).toLocaleString()}`;

		// 	event litener tombol edit dan hapus
		daftar.querySelectorAll(".edit-btn").forEach(btn => btn.addEventListener("click", () => editTransaksi(btn.dataset.id)));
		daftar.querySelectorAll(".hapus-btn").forEach(btn => btn.addEventListener("click", () => hapusTransaksi(btn.dataset.id, "Yakin ingin menghapus data ini!", true)));
	}
	...
	form.addEventListener("submit", e => {
		...
		const data = {
			...
			kategori: sanitizeInput(form.kategori.value),
			catatan: sanitizeInput(form.catatan.value)
		};

		const indexEdit = transaksi.findIndex(item => item.id.toString() === form.dataset.editing);
		if (indexEdit !== -1) {
			// Mode edit
...
function isiPilihanOption(selectEl){
	// ambil data kategori
	const dataKategori = JSON.parse(localStorage.getItem("data-kategori") || '[]');
