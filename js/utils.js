...
function elFollowMe(followerEl, event, offsetX, offsetY){
	document.addEventListener(event, (e) => {
		...
		{
			offsetX = e.clientX >= window.innerWidth - followerEl.offsetWidth ? followerEl.offsetWidth : 0;
			offsetY = e.clientY >= window.innerHeight - followerEl.offsetHeight ? -followerEl.offsetHeight : 15;
...
// 	render rows ke tabel / kontainer
	...
	if (data.length !== 0) {
		renderPaginationButtons(currentPage, totalPages, maxButton, paginationContainer, (page) => {
			paginate({
				data,
				rowsPerPage,
				currentPage: page,
...
function renderPaginationButtons(currentPage, totalPages, maxButtons, container, onPageChange) {
	container.innerHTML = "";
...
const dataSum = (data, val) => data.filter(item => item.jenis === val). reduce((item, nominal) => item + nominal.nominal, 0);
const createElement = ({
 tag = 'div',
 id = '',
 className = '',
